        ...
        <div class="row mb-4">
            <div class="col-lg-8 offset-lg-2">
                @if (!auth()->user()->packageActive())
                    ...
                    <a href="{{ url('packages') }}"><button class="btn btn-alt-primary">{{ __('Select') }}</button></a>
                @endif
                
                
                @if (auth()->user()->packageActive())
                    {{-- <h2>{{ __("Add Post") }}</h2> --}}
                    <form action="{{ route("draft.save") }}" method="POST" id="newSave">
                        @csrf
                        ...
                        @if ($step == 1)
                            <h2>{{ __("Choose Language") }}</h2>
                            <div class="mb-3">
                                <div class="row items-push">
                                    @foreach(LaravelLocalization::getSupportedLocales() as $code => $locale)
                                        <div class="col-md-4">
                                            <div class="form-check form-block">
                                                <input class="form-check-input" type="radio" value="{{ $code }}" id="draft-locale-{{ $code }}"
                                                    name="locale" @if ((empty($draft->content['code']) && $code == "ru") || (!empty($draft->content['code']) && $draft->content['code'] == $code)) checked @endif>
                                                <label class="form-check-label" for="draft-locale-{{ $code }}">
                                                    <span class="d-block fs-4 fw-semibold text-capitalize text-center my-3">
                        ...
                        @elseif ($step == 2)
                            <h2>{{ __("Title, Category, Photo") }}</h2>
                            <div class="mb-3">
                                <label class="form-label">{{ __("Title") }}</label>
                                <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" value="{{ old("title") }}" placeholder="{{ __("Title placeholder") }}">
                            ...
                            <div class="mb-3">
                                ...
                                <div id="holder" class="@error('image') is-invalid @enderror" style="margin-top:15px;max-height:100px;"></div>
                        ...
                        @elseif($step == 3)
                            <h2>{{ __("Post Content") }}</h2>
                            <div class="mb-3">
                                <label class="form-label">{{ __('Summary') }}</label>
                                <textarea name="summary" class="form-control @error('summary') is-invalid @enderror" rows="3" placeholder="{{ __("Summary placeholder") }}">{{ old("summary") }}</textarea>
                            ...
                            <div class="content-row">
                                <div class="mb-4">
                                    <label class="form-label">{{ __("Content") }} 
                                        @foreach(LaravelLocalization::getSupportedLocales() as $code => $locale)
                                            @if ($draft->getContentValue('locale') == $code)
                                                ({{ $locale['native'] }})
                                            @endif
                                        @endforeach</label>
                                    <div class="@error('content') is-invalid @enderror">
                                        <div id="content[{{ $draft->getContentValue('locale') }}]" class="editor">{!! !empty($draft->getContentvalue('content', [])[$draft->getContentValue('locale')])? $draft->getContentvalue('content', [])[$draft->getContentValue('locale')]: '' !!}</div>
                                    </div>
                                ...
                                @foreach($draft->getContentvalue('content', []) as $contentLocale => $content)
                                    @if ($draft->getContentValue('locale') != $contentLocale)
                                        <div class="mb-4">
                                            <label class="form-label">{{ __("Content") }} 
                                                @foreach(LaravelLocalization::getSupportedLocales() as $code => $locale)
                                                    @if ($contentLocale == $code)
                                                        ({{ $locale['native'] }})
                                                    @endif
                                                @endforeach</label>
                                            <div class="@error('content') is-invalid @enderror">
                                                <div id="content[{{ $contentLocale }}]" class="editor">{!! $content !!}</div>
                                            </div>
                                        </div>
                                    @endif
                                @endforeach
                            ...
                            <div class="mb-4 locale-button">
                                <div class="dropdown">
                                    <button type="button" class="btn btn-alt-secondary btn-sm dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        {{ __('Add Translate') }}
                                    </button>
                                    <div class="dropdown-menu fs-sm">
                                        @foreach(LaravelLocalization::getSupportedLocales() as $code => $locale)
                                            @if ($draft->getContentValue('locale') != $code && !in_array($code, array_keys($draft->getContentvalue('content', []))))
                                                <a href="javascript:void(0)" class="dropdown-item add-translate" data-locale="{{ $code }}" data-label="{{ $locale['native'] }}">{{ $locale['native'] }}</a>
                                            @endif
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        @elseif ($step == 4)
                            ...
                            {{-- <div class="mb-3 hidden">
                                <div class="form-check">
                                    <input type="radio" class="form-check-input" id="not-add-to-slider" name="add_to_slider" value="">
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label font-w400" data-bs-toggle="popover" data-bs-placement="right" title="" data-bs-content="{{ __("Add To Big Slider desc") }}" data-bs-original-title="{{ __("Add To Big Slider title") }}" for="add-to-big-slider">{{ __("Add To Big Slider") }}<span class="bg-primary rounded px-2 py-1 text-white fs-xs fw-semibold ms-2">{{ __("Price") }}: {{ nova_get_setting('big_slider_price') }} тг.</span></label>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label font-w400" data-bs-toggle="popover" data-bs-placement="right" title="" data-bs-content="{{ __("Add To Small Slider desc") }}" data-bs-original-title="{{ __("Add To Small Slider title") }}" for="add-to-small-slider">{{ __("Add To Small Slider") }}<span class="bg-primary rounded px-2 py-1 text-white fs-xs fw-semibold ms-2">{{ __("Price") }}: {{ nova_get_setting('small_slider_price') }} тг.</span></label>
                                </div>
                            </div>
                            ...
                            @if (auth()->user()->packageActive() && in_array(auth()->user()->package->slug, ['standart-plus', 'standart-maximum']))
                                <div class="mb-3">
                                    <div class="form-check">
                                        ...
                                        <label class="form-check-label font-w400" for="send-webpush-notifications">{{ __("Send WebPush Notifications To Fcm") }}<span class="bg-primary rounded px-2 py-1 text-white fs-xs fw-semibold ms-2">{{ __("Web Notification info") }}</span></label>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <div class="form-check">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            $(this).on('change', '[name="color"]', function() {
            ...
            }).on('click', '.add-translate', function() {
                var localeCode = $(this).data('locale');
                var localeLabel = $(this).data('label');
                $('.content-row').append(`
                    <div class="mb-4">
                        <label class="form-label">{{ __("Content") }} (${localeLabel})</label>
                        <div>
                            <div id="content[${localeCode}]" class="editor"></div>
                        </div>
                    </div>
                `);
                initTinyMCE();
                ...
                if (!$('.locale-button .add-translate').length) {
                    $('.locale-button').hide();
                }
